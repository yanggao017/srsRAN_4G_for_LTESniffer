...
inline float cf_abs(cf_t x) {
    return sqrtf(__real__ x * __real__ x + __imag__ x * __imag__ x);
}
...
int phch_worker::read_ce_abs(float *ce_abs, uint32_t tx_antenna, uint32_t rx_antenna) {
  ...
  for (i = 0; i < 12*cell.nof_prb; i++) {
      cf_t val = ue_dl.ce_m[tx_antenna][rx_antenna][i];
      ce_abs[g+i] = 20 * log10f(cf_abs(val));
    if (isinf(ce_abs[g+i])) {
      ce_abs[g+i] = -80;
    }
  }
