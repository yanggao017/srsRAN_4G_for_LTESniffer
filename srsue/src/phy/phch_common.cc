...
#include <complex>
#include <math.h>  // 提供cosf、sinf
#include <complex.h>  // 提供C复数支持（I是虚数单位）
...
phch_common::phch_common(uint32_t max_mutex_) : tx_mutex(max_mutex_)
{
  ...
  for (int i=0;i<50000;i++) {
    // 1. 计算指数的虚部（角度）
    float angle = ((float)i / 50000) * 0.1f;
    // 2. 用C标准库函数生成复数：cos(angle) + j*sin(angle)（即e^(j*angle)）
    cf_t exp_complex = 0.01f * (cosf(angle) + i * sinf(angle));
    // 3. 赋值给cf_t数组
    zeros[i] = exp_complex;
    // zeros[i] = 0.01f * std::exp(std::complex<float>(0, ((float) i/50000)*0.1f));
  }
