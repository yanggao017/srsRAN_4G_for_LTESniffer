...
int handle_scan(int argc, char* argv[]) {
    ...
    if (file_exists(cells_json) && !force) {
        std::cout << "[OK] Using cached scan result for band " << band << "\n";
    ...
    std::cout << "[START] Scanning band " << band << "...\n";
    ...
    if (cells.empty()) {
        std::cerr << "[OK] No cells found in band " << band << "\n";
        return 1;
    }
    ...
    for (const auto& cell : cells) {
        ...
        if (mib_decoded) {
            std::cout << "[OK] Valid cell confirmed: PCI=" << pci << ", MIB/SIB1/SIB2 decoded successfully\n";
            ...
            if (ret == 124) {
                std::cerr << "[ERROR] Timeout: No MIB decoded for PCI=" << pci << "\n";
            } else {
                std::cerr << "[ERROR] pdsch_ue failed or exited early (code: " << ret << ") for PCI=" << pci << "\n";
            }
            // 不保存，跳过
        }
        clear_output_dir();
    ...
    std::cout << "[OK] Saved " << cells.size() << " cells to " << cells_json << "\n";
    std::cout << "[END] Scan completed. Results saved in " << cache_dir << "\n";
...
int handle_gen_msg(int argc, char* argv[]) {
    if (argc < 1) {
        std::cerr << "Usage: lte_tool <gen-sib1-tac|gen-paging-imsi|...> -c <pci> [options]\n";
        return 1;
    }
    ...
    if (it == cmd_to_type.end()) {
        std::cerr << "[ERROR] Unknown or unsupported gen command: " << cmd << "\n";
        std::cerr << "Supported gen- commands:\n";
    ...
    int c;
    optind = 1;
    while ((c = getopt(argc, argv, "c:f:p:s:r:m:i:hv")) != -1) {
        switch (c) {
            case 'c':
    ...
    if (pid == 0) {
        ...
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
            std::cout << "[OK] Successfully generated: " << cmd
                      << " for PCI=" << cell_id << "\n";
            return 0;
        } else {
            std::cerr << "[ERROR] gen_sample exited with error\n";
            return 1;
        }
    } else {
        perror("fork");
...
// ==================== handle_over_msg: 独立处理 over-* 命令 ====================
int handle_over_msg(int argc, char* argv[]) {
    // 如果没有子命令，报错
    if (argc < 1) {
        std::cerr << "Usage: lte_tool <over-sib1-tac|over-paging-imsi|over-sib2-acbarring|over-rar|over-sib1-original|over-attach-reject|over-identity-request> -c <pci>\n";
        return 1;
    }

    // 独立的 over-* 命令到 --type 的映射
    std::map<std::string, std::string> cmd_to_type = {
        {"over-sib1-tac",           "sib1_tac"},
        {"over-paging-imsi",        "paging_imsi"},
        {"over-sib2-acbarring",     "sib2_acbarring"},
        {"over-rar",                "rar"},
        {"over-sib1-original",      "sib1_ori"},
        {"over-attach-reject",      "attach_reject"},
        {"over-identity-request",   "identity_request"},
        // 可继续扩展
    };
    ...
    auto it = cmd_to_type.find(cmd);
    if (it == cmd_to_type.end()) {
        std::cerr << "[ERROR] Unknown or unsupported over command: " << cmd << "\n";
        std::cerr << "Supported over- commands:\n";
        for (const auto& pair : cmd_to_type) {
            std::cerr << "  " << pair.first << "\n";
        }
        return 1;
    }
    std::string msg_type = it->second;
    ...
    while ((c = getopt(argc, argv, "c:h")) != -1) {
        switch (c) {
            case 'c':
                cell_id = std::stoi(optarg);
                break;
            case 'h':
                std::cout << "Usage: lte_tool " << cmd << " -c <pci>\n";
                std::cout << "Starts pdsch_enodeb to broadcast " << msg_type << " message.\n";
                return 0;
            default:
                std::cerr << "Unknown option: " << (char)c << "\n";
                return 1;
    ...
    std::vector<std::string> args;
    args.push_back("sudo");
    args.push_back("./lib/examples/pdsch_enodeb");
    args.push_back("-I");
    args.push_back("UHD");
    args.push_back("-x");
    args.push_back("1");
    ...
    std::cout << "[DEBUG] Executing: " << cmd_line << std::endl;
    ...
    if (pid == 0) {
        ...
        waitpid(pid, &status, 0);
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
            std::cout << "[OK] pdsch_enodeb completed for PCI=" << cell_id << "\n";
            return 0;
        } else {
            std::cerr << "[ERROR] pdsch_enodeb exited with error\n";
            return 1;
        }
    } else {
        perror("fork");
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cerr << "  gen-sib1-tac             -c <pci> [...]\n";
        std::cerr << "  gen-paging-imsi          -c <pci> -m <imsi>\n";
        std::cerr << "  gen-paging-sysinfmod     -c <pci> [...]\n";
        std::cerr << "  gen-sib2-acbarring       -c <pci>\n";
        std::cerr << "  gen-rar                  -c <pci> -r <rnti>\n";
        std::cerr << "  gen-sib1-original        -c <pci>\n";
        std::cerr << "  gen-attach-reject        -c <pci>\n";
        std::cerr << "  gen-identity-request     -c <pci>\n";
        // 可继续扩展
        return 1;
    }
    ...
    else if (cmd.substr(0, 4) == "gen-") {
        return handle_gen_msg(argc - 1, argv + 1);
    }
    else if (cmd.substr(0, 5) == "over-") {
        return handle_over_msg(argc - 1, argv + 1);  // 使用独立函数
    }
    else {
        std::cerr << "[ERROR] Unknown command: " << cmd << "\n";
        std::cerr << " Try 'gen-*', 'over-*', or 'scan'\n";
        return 1;
    }
}
