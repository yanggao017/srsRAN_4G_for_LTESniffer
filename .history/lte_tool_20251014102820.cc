...
int handle_scan(int argc, char* argv[]) {
    ...
    for (const auto& cell : cells) {
        ...
        if (file_exists("../output/mib.json")) {
            fs::copy_file("../output/mib.json", cell_dir + "/mib.json", fs::copy_options::overwrite_existing);
        }
        if (file_exists("../output/sib1.json")) {
            fs::copy_file("../output/sib1.json", cell_dir + "/sib1.json", fs::copy_options::overwrite_existing);
        }
        if (file_exists("../output/sib2.json")) {
            fs::copy_file("../output/sib2.json", cell_dir + "/sib2.json", fs::copy_options::overwrite_existing);
        }
    }
