...
int handle_gen_msg(int argc, char* argv[]) {
    ...
    // ✅ 自动查找 cell_<pci> 目录
    std::string cache_root = "build/cache";
    std::string cell_dir = find_cell_dir(cache_root, cell_id);

    if (cell_dir.empty()) {
        std::cerr << "❌ No cell directory found for PCI=" << cell_id
                  << " in " << cache_root << "\n";
        std::cerr << "💡 Run gen_sample first to generate cache, or check path.\n";
        return 1;
    }

    std::cout << "📡 Found cell config: " << cell_dir << "\n";
