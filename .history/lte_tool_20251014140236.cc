...
int handle_scan(int argc, char* argv[]) {
    ...
    for (const auto& cell : cells) {
        ...
        if (mib_decoded) {
            std::cout << "[OK] Valid cell confirmed: PCI=" << pci << ", MIB decoded successfully\n";
...
int handle_gen_msg(int argc, char* argv[]) {
    ...
    // ✅ 重置 getopt，准备解析从 argv[0] 开始的参数（实际是原 argv[1..n]）
    optind = 1;
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "scan") {
        ...
        // 检查是否是合法的 gen- 命令（可选：更严格的检查）
        // 例如你可以检查 cmd_to_type 中是否存在映射，但这里先直接转发
