    ...
    if (generation == "nr") {
        if (direction == "dl") {
            ...
            if (type == "ra") {
                ...
                pcap_handle->write_dl_bch_nr(mac_bytes, mac_len, PCAP_TTI);
            } else if (type == "pch") {
                std::cout << "[INFO] 类型为 PCH，写入 PCH\n";
                pcap_handle->write_dl_pch_nr(mac_bytes, mac_len, PCAP_TTI);
            } else if (type == "si_rnti") {
                std::cout << "[INFO] 类型为 SI_RNTI，写入 SI_RNTI\n";
                pcap_handle->write_dl_si_rnti_nr(mac_bytes, mac_len, PCAP_TTI);
            } else {
