    ...
    while ((opt = getopt_long(argc, argv, "hi:I:a:g:l:f:o:m:u:n:p:c:x:b:w:M:v:s:B:q:Q:E:C:T:S:P:e:t:", cfr_opts, NULL)) != -1) {
        switch (opt) {
            case 'h':
                ...
                exit(-1);
            case 'I':
                rf_dev = argv[optind-1]; // getopt 不会自动推进 optind 对于非参数选项，但 -I 后必须跟值
                break;
            case 'a':
                rf_args = optarg;
                break;
            ...
            case 'p':
                ...
                break;
            case 'c':
                cell.id = (uint32)strtol(optarg, NULL, 10);
                break;
            case 'x':
