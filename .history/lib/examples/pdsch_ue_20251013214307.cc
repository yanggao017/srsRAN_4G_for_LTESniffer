...
int main(int argc, char** argv)
{
  ...
  for (uint32_t i = 0; i < prog_args.rf_nof_rx_ant; i++)    sf_buffer[i] = srsran_vec_cf_malloc(max_num_samples);
  }
...
void handle_pdsch_pdu(srsran_pdsch_cfg_t* pdsch_cfg, uint8_t* data[SRSRAN_MAX_CODEWORDS]) {
  ...
  if (pdsch_cfg->rnti == SRSRAN_SIRNTI) {
    printf("test yg get SI\n");
    printf("tb:%d\n", nof_tb);
    for (int i=0; i<nof_tb; i++) {
      if (pdsch_cfg->grant.tb[i].enabled) {
        len = pdsch_cfg->grant.tb[i].tbs / 8;
        printf("len:%d\n", len);
        srsran_vec_fprint_byte(stdout, data[i], len);
        cbit_ref bref(&data[i], sizeof(rrc_msg));
