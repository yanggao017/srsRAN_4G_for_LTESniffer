 ...
 */
 ...
 {
   int opt;
   while ((opt = getopt_long(argc, argv, "hiIadglfmoncpqvutxbwMsBQ", cfr_opts, NULL)) != -1) {
     switch (opt) {
       case 'h':
         usage(argv[0]);
         exit(-1);
       case 'i':
         // 检查 1 到 3 个可能的文件名
         for (int i = 0; i < 3 && optind < argc && argv[optind][0] != '-'; ++i, ++optind) {
             // 检查文件名中是否包含 "paging"
             if (strstr(argv[optind], "paging")) {
                 paging_file = argv[optind];
                 if (strstr(argv[optind], "imsi")) {
                 attack_mode = "attack mode:paging with imsi [460017837217696]";
             } else {
               attack_mode = "attack mode:paging systeminfomod";
             }
 
                 paging_msg = true;
             }
             // 检查文件名中是否包含 "sib1"
             else if (strstr(argv[optind], "sib1")) {
                 sib1_file = argv[optind];
                 sib1_msg = true;
                 attack_mode = "attack mode:sib1 TAC";
             }
             // 检查文件名中是否包含 "sib2"
             else if (strstr(argv[optind], "sib2")) {
                 sib2_file = argv[optind];
                 sib2_msg = true;
                 attack_mode = "attack mode:sib2 ac-barring";
             } else if (strstr(argv[optind], "po")) {
              po_file = argv[optind];
              po_msg = true;
              attack_mode = "attack mode:pdcch order";
            } else if (strstr(argv[optind], "attach_reject")) {
              ar_file = argv[optind];
              ar_msg = true;
              attack_mode = "attack mode:attach reject";
            } else if (strstr(argv[optind], "identity_request")) {
              ir_file = argv[optind];
              ir_msg = true;
              attack_mode = "attack mode:identity request";
            }
         }
         break;
       case 'I':
