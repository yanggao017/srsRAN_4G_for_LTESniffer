...
void load_msg_buffer(uint32_t pci, inject_msg_t& msg)
{
  ...
  if (cell_dir == nullptr) {
    throw std::runtime_error("failed to find cell cache directory for PCI=" + std::to_string(pci) +
                             " under " + g_cache_root);
  }
  ...
  if (fp == nullptr) {
    std::string err = "failed to open inject file: " + std::string(msg.file_name) + " (path: " + file_path + ")";
    free(file_path);
...
void prepare_inject_msgs(uint32_t pci, uint32_t samples_per_subframe)
{
  ...
  if (g_args.inject_type == "paging_imsi") {
    ...
    g_inject_msgs.push_back({"mib_dlband", "mib_dlband_sf990.fc32", 0, nullptr, 0, 0, {}});
  } else {
