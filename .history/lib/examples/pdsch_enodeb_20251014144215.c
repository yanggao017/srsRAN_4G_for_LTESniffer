            ...
            case 't': {
              attack_mode = optarg;
                if (attack_mode == "paging_imsi") {
                    ...
                    ar_msg = true;
                    attachreject_file_name = "attach_reject_sf.fc32"
                }
...
int read_cell_config_from_json() {
    ...
    fclose(file);
    buffer[read_size] = '\0';  // 确保以 null 结尾
    ...
    do {
        ...
        cell.id = (uint32_t)item->valuedouble;  // cJSON 用 double 存数字
    ...
    } while (0);  // 使用 do-while(0) 实现 goto 风格的错误退出
...
int main(int argc, char** argv)
 {
  ...
  if (cell_dir == NULL || strlen(cell_dir) == 0) {
      ...
      if (cell_dir) free(cell_dir); // 防止 find_cell_dir 内部 malloc 了部分内存
      return -1;
  }

  // 步骤 2: 使用 cell_dir 拼接各个文件路径
  char mib_path[PATH_MAX];
  char sib1_path[PATH_MAX];
  char sib2_path[PATH_MAX];

  snprintf(mib_path, sizeof(mib_path), "%s/mib.hex", cell_dir);
  snprintf(sib1_path, sizeof(sib1_path), "%s/sib1.hex", cell_dir);
  snprintf(sib2_path, sizeof(sib2_path), "%s/sib2.hex", cell_dir);
  snprintf(cell_config_path, sizeof(cell_config_path), "%s/cell.json", cell_dir);

  // 步骤 3: 使用完 cell_dir 后释放（重要！）
  // 注意：只有在后续不再需要 cell_dir 字符串时才释放
  // 如果后续还要用，可以 strdup 保存一份

  //free(cell_dir); // ✅ 释放 find_cell_dir 中 malloc 的内存
