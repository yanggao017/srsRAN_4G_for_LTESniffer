...
#define MAX_TX_SCHEDULES 8
#define TX_PERIOD_S 0.010
#define TX_HORIZON_S 0.003
#define TX_RX_DELAY_S 0.020
#define TX_TIMESTAMP_ADVANCE_S (66.0 / 30720000.0)
...
typedef struct {
  ...
  uint32_t           target_tti;
  bool               enabled;
  srsran_timestamp_t next_tx_time;
} tx_schedule_entry_t;

static tx_schedule_entry_t tx_schedules[MAX_TX_SCHEDULES];
static size_t              nof_tx_schedules = 0;
...
static bool mib_ready = false;
static bool schedule_trigger_pending = false;
static uint32_t sfn;
static int sf_idx;
 ...
 static bool        use_standard_lte_rate = true;
...
{
    int opt;
    while ((opt = getopt_long(argc, argv, "h:i:I:a:g:l:f:o:m:u:n:p:c:x:b:w:M:v:s:B:qQE:C:T:S:P:e:t:", cfr_opts, NULL)) != -1) {
        switch (opt) {
            case 'h':
                usage(argv[0]);
                exit(-1);
...
static void add_tx_schedule(const char* name, cf_t** buffer, uint32_t subframe)
{
  if (nof_tx_schedules >= MAX_TX_SCHEDULES) {
    ERROR("Too many TX schedules configured");
    exit(-1);
  }

  tx_schedules[nof_tx_schedules].name       = name;
  tx_schedules[nof_tx_schedules].buffer     = buffer;
  tx_schedules[nof_tx_schedules].target_tti = subframe;
  tx_schedules[nof_tx_schedules].enabled    = true;
  srsran_timestamp_init(&tx_schedules[nof_tx_schedules].next_tx_time, 0, 0.0);
  nof_tx_schedules++;
}

static void configure_tx_schedules()
{
  nof_tx_schedules = 0;

  if (sib1_msg) {
    add_tx_schedule("sib1", sib1_buffer, 5);
  }
  if (sib2_msg) {
    add_tx_schedule("sib2", sib2_buffer, 1);
  }
  if (paging_msg) {
    add_tx_schedule("paging", paging_buffer, 9);
  }
  if (po_msg) {
    add_tx_schedule("pdcch_order", po_buffer, 8);
  }
  if (ar_msg) {
    add_tx_schedule("attach_reject", ar_buffer, 8);
  }
  if (ir_msg) {
    add_tx_schedule("identity_request", ir_buffer, 8);
  }
  if (ext_msg) {
    add_tx_schedule("extend", ext_buffer, target_tti);
  }

  if (nof_tx_schedules == 0) {
    ERROR("No TX schedule configured. Please select a valid --type.");
    exit(-1);
  }
}

static void init_tx_schedules_from_sync(const srsran_timestamp_t* sync_time, int cur_sf_idx)
{
  for (size_t i = 0; i < nof_tx_schedules; i++) {
    tx_schedule_entry_t* entry = &tx_schedules[i];
    int                  delta_sf;

    memcpy(&entry->next_tx_time, sync_time, sizeof(entry->next_tx_time));
    delta_sf = 10 + (int)entry->target_tti - cur_sf_idx;
    if (delta_sf <= 0) {
      delta_sf += 10;
    }

    srsran_timestamp_add(&entry->next_tx_time, 0, TX_RX_DELAY_S + delta_sf * 0.001 - TX_TIMESTAMP_ADVANCE_S);
    printf("[TX schedule] %s -> sf=%u time=%ld + %.9f\n",
           entry->name,
           entry->target_tti,
           (long)entry->next_tx_time.full_secs,
           entry->next_tx_time.frac_secs);
  }
}

static void send_due_tx_schedules(const srsran_timestamp_t* cur_time)
{
  srsran_timestamp_t horizon;

  memcpy(&horizon, cur_time, sizeof(horizon));
  srsran_timestamp_add(&horizon, 0, TX_HORIZON_S);

  for (size_t i = 0; i < nof_tx_schedules; i++) {
    tx_schedule_entry_t* entry = &tx_schedules[i];

    while (entry->enabled && srsran_timestamp_compare(&entry->next_tx_time, &horizon) <= 0) {
      int ret = srsran_rf_send_timed_multi(&radio,
                                           (void**)entry->buffer,
                                           sf_n_samples,
                                           entry->next_tx_time.full_secs,
                                           entry->next_tx_time.frac_secs,
                                           true,
                                           true,
                                           true);
      if (ret != sf_n_samples) {
        ERROR("Timed TX failed for %s: expected %u samples got %d", entry->name, sf_n_samples, ret);
        exit(-1);
      }

      printf("[TX] %s sf=%u at %ld + %.9f\n",
             entry->name,
             entry->target_tti,
             (long)entry->next_tx_time.full_secs,
             entry->next_tx_time.frac_secs);
      srsran_timestamp_add(&entry->next_tx_time, 0, TX_PERIOD_S);
    }
  }
}
...
static void* tx_thread_func() {
  ...
  while (!go_exit) {
    ...
    if (cur_rx_ret != 0 || !schedule_trigger_pending) {
      continue;
    }
    ...
    if (!schedule_locked) {
      init_tx_schedules_from_sync(&cur_time, cur_sf_idx);
      schedule_locked = true;
      schedule_trigger_pending = false;
    }

    send_due_tx_schedules(&cur_time);
   }
