...
// 核心函数：查找 cell_<pci> 目录并验证 mib.json
char* find_cell_dir(const char* cache_root, uint32_t pci) {
    ...
    // 使用 opendir/recursiveness 手动递归（C 不支持 std::filesystem）
    // 我们写一个辅助函数进行 DFS 遍历
    void search_recursive(const char* root) {
            ...
            if (is_directory(subpath)) {
                ...
                if (strcmp(entry->d_name, target_name) == 0) {
                    ...
                    if (mib_path && file_exists(mib_path)) {
                        if (!found_path) {
                            found_path = strdup(subpath);  // 复制路径返回
                        } else {
                            // 多个匹配，只保留第一个，打印警告
                ...
                // 递归进入子目录（即使当前是目标目录也继续找？但按需求一般唯一）
                search_recursive(subpath);
            }
            free(subpath);
        }
