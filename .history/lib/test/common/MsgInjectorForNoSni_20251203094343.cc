...
MsgInjector::MsgInjector(const InjectorConfig& cfg_, LTESniffer_Core* owner_)
    ...
    attack_type(AttackType::ATTACH_REJECT), tti_offset(0), inject_count(1)
{
  ...
  worker = std::thread(&MsgInjector::workerLoop, this);
}
...
std::vector<InjectTask> MsgInjector::generateTasks(const MessageGenRequest& req)
{
...
void MsgInjector::scheduleAndSendTasks(const std::vector<InjectTask>& tasks, bool blocking)
{
...
void MsgInjector::setAttackType(AttackType at) { attack_type = at; }
void MsgInjector::setEMMCause(int ec) { emm_cause = ec; }
void MsgInjector::setESMCause(int ec) { esm_cause = ec; }

void MsgInjector::setTtiOffset(int offset) { tti_offset = offset; }
void MsgInjector::setInjectCount(int count) { inject_count = count; }
void MsgInjector::setTtiInterval(int interval) { tti_interval = interval; }
...
void MsgInjector::generate_signal_part1() {
...
void MsgInjector::handle_pdu_signalgen_part2(uint32_t rnti, uint32_t target_tti, cf_t** signal, int inject_index) {
...
void MsgInjector::generate_signal_part2(uint32_t rnti, uint32_t tti, int inject_index)
{
  sf_cfg_dl.tti = tti;
