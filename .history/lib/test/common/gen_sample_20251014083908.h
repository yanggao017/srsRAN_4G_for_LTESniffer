...
int gen_sib1_tac(uint8_t* buffer, uint32_t buffer_len, uint32_t* msg_len, int tac) {
  //pci 1
    //const char *rrc_msg_hex_stream = "4040040300070019b0181400108280000000";
//pci 420
...
/*
    ...
    for (size_t i = 0; i < byte_array_length; i++) {
        printf("0x%02x ", rrc_msg[i]);
    }
    printf("\n");
*/
  //uint8_t rrc_msg[] = {0x40, 0xd1, 0x80, 0x23, 0xa3, 0x00, 0x06, 0x30, 0x46, 0xcc, 0xa3, 0x80, 0x50, 0x30, 0x02, 0x21, 0x02, 0x08, 0x98, 0x87, 0x07, 0x86};
  uint32_t rrc_msg_len = sizeof(rrc_msg);
  cbit_ref bref(&rrc_msg[0], sizeof(rrc_msg));
...
int gen_sib2_acbarring(uint8_t* buffer, uint32_t buffer_len, uint32_t* msg_len) {
    const char *rrc_msg_hex_stream = "00821c31286fe12bd0608664601000020090ee316aa53730d07121000c0064f49546e93c0504ee97a480";
...
/*
// 新增：验证生成的 SIB1 消息
  bcch_dl_sch_msg_s verify_msg;
  cbit_ref verify_bref(buffer, *msg_len); // 创建比特流引用，指向已生成的 buffer

  // 解包到新的结构体中
  if (verify_msg.unpack(verify_bref) != SRSRAN_SUCCESS) {
    ERROR("Error unpacking SIB1 message for verification");
    return -1;
  }
  // 访问并打印 SIB1 消息中的字段
  sib_type2_s& verified_data = (&verify_msg.msg.c1().sys_info().crit_exts)->sys_info_r8().sib_type_and_info[0].sib2();

  int ret_ver = print_sib2_fields(verified_data);
  if (ret_ver){
    printf("!!!success print!!!\n");
  } else {
    printf("!!!failed print!!!\n");
...
int gen_identity_request(uint8_t* buffer, uint32_t buffer_len, uint32_t* msg_len) {
  //const char *rrc_msg_hex_stream = "210d1fa000000800183aa808000000000000";
