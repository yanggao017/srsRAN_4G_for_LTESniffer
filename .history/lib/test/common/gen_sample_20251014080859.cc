...
const char* outputfile = "output.fc32";
static FILE *fp = NULL;
...
void generate_message(uint8_t* payload[], uint32_t* payload_len);
...
void usage(char *prog) {
  ...
  // 示例
  printf("\nExamples:\n");
  printf("  Paging System Info Mod: %s --type paging_sysinfmod -r 0xfffe -s 9 -o out_paging_sysinfmod -p 100 -c 420\n", prog);
  printf("  Paging with IMSI:       %s --type paging_imsi -r 0xfffe -s 9 -o out_paging_imsi -p 100 -c 420 -m 460017837217696\n", prog);
...
void parse_args(int argc, char **argv) {
  ...
  // 使用 getopt_long 支持 --type
  while ((opt = getopt(argc, argv, "c:f:p:s:r:o:m:i:hv")) != -1) {
    switch (opt) {
      case 'c':
      ...
      case 'r':
        ...
        break;
      case 'o':
        strncpy((char *)outputfile, optarg, sizeof(outputfile)-1);
  ...
  for (int i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "--type") == 0) {
      ...
      attack_type = get_attack_type_from_name(argv[i+1]);
      if (attack_type == -1) {
        fprintf(stderr, "❌ Invalid attack type: '%s'\n", argv[i+1]);
        fprintf(stderr, "Run '%s' to see valid types.\n", argv[0]);
        exit(-1);
      }
      type_set = true;
      break;
...
int main(int argc, char** argv) {
  ...
  generate_message(payload, &payload_len);
...
// 假设你已定义枚举（前面已定义）
// enum AttackType { PAGING_SYSINFOMOD, PAGING_IMSI, ..., ATTACK_TYPE_COUNT };
