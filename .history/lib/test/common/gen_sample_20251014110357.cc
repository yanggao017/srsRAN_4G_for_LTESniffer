...
int main(int argc, char** argv) {
  ...
  const std::string cache_root = "build/cache";  // 你的缓存根目录
  std::string cell_dir = find_cell_dir(cache_root, cell.id);

  if (cell_dir.empty()) {
      fprintf(stderr, "❌ Error: No valid cell_%u directory found in %s\n", cell.id, cache_root.c_str());
      fprintf(stderr, "Please run cell search and pdsch_ue first.\n");
      return -1;
  }
  ...
  printf("📡 Found cell config: PCI=%u\n", cell.id);
 ...
 */
int read_cell_config_from_json() {
  std::ifstream file("../output/cell.json");
  if (!file.is_open()) {
      std::cerr << "❌ Error: Cannot open file '../output/cell.json'\n";
      return -1;
  }
  json j;
  try {
  ...
  try {
      ...
      switch (phich_resources) {
          ...
          default:
              std::cerr << "❌ Invalid PHICH Resources value: " << phich_resources << "\n";
              return -1;
      }

  } catch (const json::out_of_range& e) {
      std::cerr << "❌ Missing required field in JSON: " << e.what() << "\n";
      return -1;
  } catch (const json::type_error& e) {
      std::cerr << "❌ Type error in JSON: " << e.what() << "\n";
      return -1;
  }
  std::cout << "✅ Successfully loaded cell config:\n";
...
std::string find_cell_dir(const std::string& cache_root, uint32_t pci) {
  ...
  try {
      for (const auto& entry : fs::recursive_directory_iterator(cache_root)) {
          if (entry.path().filename() == target && entry.is_directory()) {
              std::string mib_path = entry.path().string() + "/mib.json";
              if (fs::exists(mib_path)) {  // ✅ 使用 Boost 的 exists
                  if (result.empty()) {
                      result = entry.path().string();
                  } else {
                      std::cerr << "⚠️ Multiple cell directories found for PCI=" << pci << ":\n";
                      std::cerr << "   " << result << "\n";
