...
int handle_over_msg(int argc, char* argv[]) {
    // 如果没有子命令，报错
    if (argc < 1) {
        std::cerr << "Usage: lte_tool <over-sib1-tac|over-paging-imsi|over-paging-sysinfmod|over-sib2-acbarring|over-rar|over-sib1-original|over-attach-reject|over-identity-request> -c <pci>\n";
        return 1;
    }
    ...
    std::map<std::string, std::string> cmd_to_type = {
        ...
        {"over-paging-sysinfmod",   "paging_sysinfmod"},
