...
void run_sync_loop(srsran_cell_t cell, float search_cell_cfo)
{
  ...
  while (!g_go_exit && (g_args.nof_subframes < 0 || (int)printed < g_args.nof_subframes)) {
    ...
    ret = srsran_ue_sync_zerocopy(&ue_sync, sf_ptrs, max_num_samples);
    if (ret < 0) {
    ...
    printf("[sync] sfn=%4u sf=%u rx_time=%.6f cfo=%.1f Hz sfo=%.3f\n",
           mib_valid ? mib_sfn : 0,
...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    printf("[mib] pci=%u sfn=%u prb=%u ports=%u cp=%s\n",
           ...
           cell.nof_ports,
           cell.cp == SRSRAN_CP_NORM ? "Normal" : "Extended");
    run_sync_loop(cell, search_cell_cfo);
    g_radio->stop();
