...
static void print_future_tx(uint32_t subframe, int next_sfn, const srsran_timestamp_t* future_time)
{
  printf("%s [Subframe %u] [future_time] next_sfn: %d time: %.6f s\n",
         attack_mode ? attack_mode : "tx",
         subframe,
         next_sfn,
         (double)future_time->full_secs + future_time->frac_secs);
}
     ...
     if (cur_rx_ret == 0 && cur_sfn >= 0) {
       ...
       if (sib1_msg && paging_msg) {
        if (next_sfn % sib1_period == 0 || true) {
          ...
          print_future_tx(5, next_sfn, &future_time);
          ...
          time_offset = (10 + target_tti - cur_sf_idx) * 0.001 - 0.0001;
          srsran_timestamp_add(&future_time, 0, time_offset - (66.0 / 30720000.0));
          print_future_tx(9, next_sfn, &future_time);
        ...
        if (next_sfn % sib2_period == 0) {
          ...
          time_offset = (10 + target_tti - cur_sf_idx) * 0.001 - 0.0001;
          srsran_timestamp_add(&future_time, 0, time_offset - (66.0 / 30720000.0));
          print_future_tx(target_tti, next_sfn, &future_time);
